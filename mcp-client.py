...
ITEM_TO_RESEARCH = "BT Speaker"
OUTPUT_REPORT_NAME = "BT-Speaker_Report_4Jun9pm"



# Define the initial prompt for the web automation task
INITIAL_PROMPT = "Search the price of "+ITEM_TO_RESEARCH+" and write a summary of your findings in markdown format to a file named '"+OUTPUT_REPORT_NAME+"'.txt'"
...
#NOTE: BEFORE YOU RUN MODEL
AWS_CREDS = {
    # 1. ADD YOUR ACCESS KEYS
    # 2. ADD YOUR SECRET ACCESS KEYS

}
# Model ID for summarization (using a smaller model for efficiency)
SUMMARY_MODEL_ID = "eu.anthropic.claude-3-7-sonnet-20250219-v1:0"#"us.anthropic.claude-3-5-haiku-20241022-v1:0"
    ...
    try:
        ...
        async with stdio_client(server_params) as (read, write):
            ...
            async with ClientSession(read, write) as session:
                ...
                bedrock_client = boto3.client('bedrock-runtime',region_name='eu-north-1',**AWS_CREDS)
        ...
        print(f"Error: {str(e)}")
        traceback.print_exc()
        print("Try running the server first with: python mcp-server.py")
